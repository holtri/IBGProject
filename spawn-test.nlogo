...
patches-own[spawn-point obstacle]
...
commuters-own[exit-path stuck-count]
...
to setup
  ...
  [
    ...
    ask doors [
      ...
      set obstacle true
      sprout-train-doors 1 [hide-turtle]
    ]
  ]
...
to go
  ...
  if(ticks mod 60 = 0)[
    ...
    ask patches with [spawn-point = true][
      sprout-commuters random (11) [set shape "person" set color blue set exit-path a-star-nlogo:a-star-search self targets
  ...
  ask exits[
    repeat 2 [
      ...
      [ask first exit-queue
        [
          ;show (word "to die: " self)
          die
        ]
...
to move-one-step
  ...
  ask commuters[
    ...
    [
      let next-step first exit-path      
      
      ifelse(not any? other commuters-on next-step)
      [
        set exit-path but-first exit-path
        move-to next-step
      ]
      ...
      if(any? exits-on next-step)[
        set exit-path []
        enqueue-for-exit ([exits-here] of next-step)
      ]
    ]
  ]
...
to avoid-commuter
  ...
  [
    ...
    set exit-path a-star-nlogo:a-star-search-with-turtle-avoidance self targets next-step
  ]
end

to enqueue-for-exit [exit-to-queue]  
  ask exit-to-queue[
    set exit-queue lput myself exit-queue
  ]  
end

to-report report-exit-queue [exit-number]
  report [exit-queue] of exit exit-number 
end
@#$#@#$#@
GRAPHICS-WINDOW
373
