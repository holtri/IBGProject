...
commuters-own[exit-path stuck-count direction enqueued target-door]
patches-own[spawn-point obstacle entry-point pdirection]
exits-own[exit-queue]
train-doors-own[direction exit-queue d-timer t-timer open?]
entries-own[door-rating]
...
to setup
  ...
  ;setup globals  
  set train-arrival-interval 150
  set door-opening-time 40
  
  ;setup simple station layout
  
  ;exits
  let exit-doors (patch-set patch min-pxcor 1 patch max-pxcor 1)
  ask exit-doors[
    set plabel "EXIT"
    set pcolor green
  ...
  [
    ...
    ifelse (pycor = max-pycor)[
      ask patch-at -1  -1 [set pcolor red sprout-train-doors 1 [
          set open? false set t-timer train-arrival-interval / 2 set d-timer -1 set direction 0 set exit-queue [] hide-turtle]]
      ask patch-at  1  -1 [set pcolor red sprout-train-doors 1 [
          set open? false set t-timer train-arrival-interval / 2 set d-timer -1 set direction 0 set exit-queue [] hide-turtle]]
    ][
    ask patch-at -1  1 [set pcolor red sprout-train-doors 1 [
        set open? false set t-timer train-arrival-interval set d-timer -1 set direction 1 set exit-queue [] hide-turtle]]
    ask patch-at  1  1 [set pcolor red sprout-train-doors 1 [
        set open? false set t-timer train-arrival-interval set d-timer -1 set direction 1 set exit-queue [] hide-turtle]]
    ]
  ]
  ;entries
  let entry-doors (patch-set patch min-pxcor -1 patch max-pxcor -2)
  ask entry-doors[
    set pcolor green
    set plabel "ENTRY"
  ...
  ;obstacles
  foreach n-values 30 [?]  [
    ask patch (min-pxcor + ?) 0 [set pcolor grey set obstacle true]
  ]
...
to update-door-timer
  let next-tta int random-normal train-arrival-interval 20
  foreach n-values 2 [?][
    ask train-doors with [direction = ?][
      if(t-timer = 0)[
        ifelse(open?)[set open? false set pcolor red set t-timer next-tta][set open? true set pcolor green set t-timer door-opening-time]
      ]
...
to sprout-new-commuters
  ...
  foreach n-values 2 [?][
    if(any? train-doors with [direction = ? and open?])[    
      ask patches with [spawn-point = true and pdirection = ?][
        if(random-float (1) > 0.98) [sprout-commuters 1 [set size 2 set shape "person" set color blue 
  ...
  if(ticks mod 10 = 0)[
    ask patches with [entry-point = true][
      let commuter-dir 0
      ...
      sprout-commuters random (3) [set size 2 set shape "person" set color white set exit-path a-star-nlogo:a-star-search self targets set enqueued false set target-door target set direction "into-train" set exit-path but-first exit-path]
    ]
  ]
end
...
to move
  ...
  [
    ...
    [
      ...
      [        
        if(direction = "into-train" and not any? train-doors-on first exit-path)
        [
    ...
    if(direction = "into-train" and enqueued and [open?] of target-door and length exit-path > 1)[set enqueued false]
  ]        
end
...
to enqueue-if-queue-found
  ...
  set nsc nsc with [direction = "into-train" and enqueued = true]
...
to avoid-commuter
  ...
  [
    let targets exits 
    if(direction = "into-train")[set targets (turtle-set target-door)]
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot count commuters"
