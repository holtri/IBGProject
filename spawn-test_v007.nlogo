...
;breed [train-doors-inside train-door-inside]

commuters-own[exit-path stuck-count direction enqueued target-door speed speed-count]
patches-own[spawn-point obstacle entry-point pdirection]
exits-own[exit-queue]
train-doors-own[direction exit-queue t-timer open?]
entries-own[tta]
...
to sprout-new-commuters
  ...
  foreach n-values 2 [?][
    if(any? train-doors with [direction = ? and open?])[
      ...
      ask patches with [spawn-point = true and pdirection = ?][
        ...
        [sprout-commuters random-poisson(lambda-arrival-disembark) [set size 2 set shape "person" set-speed
  ...
  ask entries[
    ifelse(tta = 0)[    
      ...
      ask patch-here [sprout-commuters random-poisson(lambda-arrival-embark) [ set size 2 set shape "person" set exit-path a-star-nlogo:a-star-search self targets set enqueued false set target-door target set direction "into-train" set exit-path but-first exit-path set-speed]]
      set tta int random-exponential(1 / lambda-arrival-embark)
    ]
    [set tta tta - 1]
  ]
